...
class TrackMaze(Maze):
    ...
    def generate(self, number, noise_scale, gaps):
        ...
        if self.four_dof:
            left_indices = np.where(goal_locations[:, 1] == 1)[0]
            right_indices = np.where(goal_locations[:, 1] == self.size - 2)[0]

            n_rotate_left = len(left_indices) // 2
            n_rotate_right = len(right_indices) // 2

            rotated_indices = set(np.random.choice(left_indices, n_rotate_left, replace=False)) | set(np.random.choice(right_indices, n_rotate_right, replace=False))
        ...
        for n in range(number):
            ...
            if self.four_dof and n in rotated_indices:
                maze, goal_locations[n] = self.rotate_maze(maze, goal_locations[n])
    ...
    def rotate_maze(self, maze, goal_location):
        """Generate a horizontal maze with the same properties."""
        new_maze = np.rot90(maze, k=1, axes=(0, 1))

        row, col = goal_location
        if col == 1: 
            new_goal = [self.size - 2, self.size // 2]
        elif col == self.size - 2:  
            new_goal = [1, self.size // 2]
            
        return new_maze, new_goal
