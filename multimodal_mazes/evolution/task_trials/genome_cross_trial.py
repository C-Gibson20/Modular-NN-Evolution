import pickle
from multimodal_mazes.image_classification.image_classification import CrossClassification
...
if __name__ == "__main__":
    ...
    img = CrossClassification(size=9)
    img.generate(number=10)

    ea = GenomeEA(hyperparameters=HYPERPARAMETERS)
    ea.generate(img, sensor_noise_scale=0.0)

    run_genome_exp(ea, 2)
    # run_genome_exp(ea, 250)

    trial_objects = {'ea': ea, 'img': img, 'hyperparameters': HYPERPARAMETERS}

    ea._pool.shutdown(wait=False)
    ea._pool = None

    with open("genome_img_cross.pkl", "wb") as f:
